...
const addGeneralQuestion = async (db, mg, snap) => {
    ...
    await asyncForEach_1.default(userData.docs, async (doc) => {
        const user = doc.data();
...
exports.default = addGeneralQuestion;
const checkIfGetEmailNotification = async (uid, db) => {
    ...
    if (settings.getNewQuestionNotification === undefined)
        return true;
    if (settings.getNewQuestionNotification === true)
        return true;
    return false;
};
//# sourceMappingURL=addGeneralQuestion.js.map