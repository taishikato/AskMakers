...
const authUserFunc = async (router: NextRouter, dispatch) => {
  ...
  if (result.user !== null) {
    // 公開
    const publicUsersRef = db.collection('publicUsers').doc(result.user.uid);
    // 秘密
    const secretUsersRef = db.collection('secretUsers').doc(result.user.uid);
    if (result.additionalUserInfo.isNewUser) {
      ...
      if (result.additionalUserInfo.providerId === 'twitter.com') {
        newPublicUserData.social.twitter = result.additionalUserInfo.username;
        newPublicUserData.tagline =
          result.additionalUserInfo.profile.description;
      }
